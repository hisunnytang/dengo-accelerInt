...
void initialize_gpu_memory(int padded, mechanism_memory** h_mem, mechanism_memory** d_mem, cvklu_data** h_chem_data)
{
  ...
  cudaErrorCheck( cudaMalloc(&((*h_mem)->y),  NN * padded * sizeof(double)) );
  cudaErrorCheck( cudaMalloc(&((*h_mem)->dy), NN * padded * sizeof(double)) );
  ...
  //jacobian is not needed yet: cudaErrorCheck( cudaMalloc(&((*h_mem)->jac), NN * NN * padded * sizeof(double)) );
  
  // initialize the arrays with values
  cudaErrorCheck( cudaMemset((*h_mem)->dy, 0, NN * padded * sizeof(double)) );
